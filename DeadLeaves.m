...
stimuli_paths = "C:\Users\danielgurman\OneDrive - Meta\Documents\GitHub\margaret-river\VBD Sensitivity\assets\Backgrounds\Noise\lab\deadleaves\";

numIMs = 50;
% sz = 2560;
ops.shape = "square";
sz = 3840;
for p = 1:length(stimuli_paths)
    stimuli_path = stimuli_paths(p);
    for i = 1:numIMs
        im = compute_dead_leaves(sz,3,ops);
        imwrite(im, fullfile(stimuli_path, sprintf('DeadLeaves_%d.png', i)), 'png');
    end
end
...
function M = compute_dead_leaves(n,sigma,options)
...
if isfield(options,'rmin')
    ...
    rmin = 0.001;    % maximum proba for rmin, shoult be >0
end
if isfield(options,'rmax')
    rmax = options.rmax;
else
    rmax = .8;    % maximum proba for rmin, shoult be >0
end
...
k = 400;        % sampling rate of the distrib
